 ...
 */
public class NGramBasedDistance implements NormalizedStringDistance {

	private final int n;

	public NGramBasedDistance(int n) {
		this.n = n;
	}

	public NGramBasedDistance() {
		this.n = 2;
	}
		...
		for (j = 1; j <= tl; j++) {
			...
			for (i = 1; i <= sl; i++) {
				...
				for (int ni = 0; ni < n; ni++) {
					if (sa[i - 1 + ni] != t_j[ni]) {
						cost++;
					} else if (sa[i - 1 + ni] == special) { // discount matches
															// on prefix
						tn--;
					}
				}
	...
	{
		NGramBasedDistance ngram = new NGramBasedDistance();
		System.out.println(ngram.distance("cleveland cavaliers", "cleveland cavaliers"));
	}
}